                        ...
                        <div style="position:relative;width: 96%;margin-left: 2%">
                            <div class="message-input">
                                <input
                                    ...
                                    type="text"
                                    placeholder="Send a message. Use hotkeys or Markdown to format text (ie. Ctrl+B for bolding)."
...
function joinRoom() {
    ...
    current_room.onJoin((room, nickname) => {
        ...
        `<div style="font-size:12px;display:flex;justify-content:center;color:grey"><span>${nickname} joined ${room.name}</span></div>`;
    ...
    document.querySelector('.chat-messages').addEventListener('click', function (event) {
        ...
        msgInputElement.addEventListener("keydown", (e) => {
            ...
            if (e.key === "Enter") {
                ...
                if (message !== "") {
                    current_room.send(message);
                    message_list[current_room_name].push({ 
                        sender: nickname, 
                        status: wt.registeredNickname, 
                        message: message, 
